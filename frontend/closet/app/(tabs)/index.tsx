...
const FILTER_COLORS  = ["Black","White","Grey","Brown","Beige","Red","Pink","Purple","Blue","Navy","Green","Yellow","Orange","Gold","Mint","Cream"];
const FILTER_SEASONS = ["Spring","Summer","Autumn","Winter"];
const FILTER_SIZES   = ["XXS","XS","S","M","L","XL","XXL","One Size"];
...
const COLOR_HEX: Record<string, string> = {
  ...
  Green:"#43A047", Yellow:"#FDD835", Orange:"#FB8C00", Gold:"#FFD700", Mint:"#80CBC4", Cream:"#FFF8E1",
};
...
export default function WardrobeScreen() {
  ...
  const filtered = items.filter(item => {
    ...
    if (filters.subcategories.length > 0 &&
        !filters.subcategories.some(sc => item.tags?.map(t=>t.toLowerCase()).includes(sc.toLowerCase())))
      return false;
    if (filters.colors.length > 0 && !filters.colors.some(c => item.colors?.includes(c))) return false;
    if (filters.seasons.length > 0 &&
        !filters.seasons.some(se => item.tags?.map(t=>t.toLowerCase()).includes(se.toLowerCase())))
      return false;
      ...
      <View style={s.body}>
        ...
        <View style={s.controlsCard}>
          ...
          <View style={s.searchRow}>
            ...
            <TouchableOpacity style={s.iconBtn}><Text style={{ fontSize: 14 }}>⭐</Text></TouchableOpacity>
            <TouchableOpacity style={s.iconBtn}><Ionicons name="eye-off-outline" size={15} color="#555" /></TouchableOpacity>
            {/* ── Filter button ── */}
            <TouchableOpacity
              style={[s.iconBtn, activeFilterCount > 0 && s.iconBtnActive]}
              onPress={() => setShowFilter(true)}
            >
              <Feather name="sliders" size={15} color={activeFilterCount > 0 ? "#fff" : "#555"} />
              {activeFilterCount > 0 && (
                <View style={s.badge}><Text style={s.badgeTxt}>{activeFilterCount}</Text></View>
              )}
            </TouchableOpacity>
          </View>
        </View>
        ...
        {activeFilterCount > 0 && (
          <ScrollView horizontal showsHorizontalScrollIndicator={false}
            contentContainerStyle={{ paddingHorizontal: 14, paddingVertical: 8, gap: 8, backgroundColor: "#fff" }}>
            {filters.category !== "" && (
        ...
        {items.length === 0 ? (
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity
                style={[s.gridItem, { backgroundColor: item.bg }]}
                onPress={() => router.push({ pathname: "/item-detail" as any, params: { itemJson: JSON.stringify(item) } })}
              >
                {item.image
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 24 }}>
              ...
              <View style={fc.row}>
                {FILTER_SEASONS.map(season => (
                  <Chip key={season} label={`${SEASON_EMOJI[season]} ${season}`}
              ...
              <View style={fc.row}>
                {FILTER_SIZES.map(sz => (
                  <Chip key={sz} label={sz} active={filters.sizes.includes(sz)}
